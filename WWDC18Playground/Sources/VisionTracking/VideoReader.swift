...
internal class VideoReader {
  ...
  private var cachedFirstFrame: CVPixelBuffer?
  ...
  func restartReading() -> Bool {
    ...
    if self.assetReader.startReading() {
      self.cachedFirstFrame = self.nextFrame()
    } else {
      self.cachedFirstFrame = nil
    }

    return self.cachedFirstFrame != nil
  }
  ...
  func fisrtFrame() -> CVPixelBuffer? {
    return self.cachedFirstFrame
  }
}
